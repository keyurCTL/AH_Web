...
import { firstLetterCapital, getCategoriesNSubCategories } from '@/lib/utils'
        ...
        <section className="tours-section">
            <div className="container">
                <div className="tours-content">
                    <div className="title">
                        <h4>TOTAL TOUR: <span>({totalBasePackages})</span></h4>
                    </div>
                        ...
                        <div className="filter">
                            <select name="explore-tour" id="explore-tour" value={selectedValue} onChange={handleExploreDDChange}>
                                ...
                                {subCategoryOptions?.map((item: string, index) => (
                                    <option key={index} value={item}>{firstLetterCapital(item?.replace(/-+/g, " "))}</option>
                                ))}
                            </select>
                        </div>
                        {selectedValue != "" ?
                    ...
                    <div className="tour-cards">
                        <div className="row">
                            {packages != null ? Object.keys(packages)?.map((item, index) => {
                                const packageImg = packages[item][0]?.navbar?.img?.file_public_url || null
                                const totalPackages = packages[item]?.length
                                return (
